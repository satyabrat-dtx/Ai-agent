# DB2ADMIN.COSTSFROMACCPERCOSTCENTERBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 86780
...
| 2 | `COSTCENTERCODE` | CHAR(20) |  |  |  |  |
| 3 | `TOTALCOST` | DECIMAL(15,5) |  |  |  |  |
...
| 15 | `TOTALCOSTSNDCUR` | DECIMAL(15,5) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `COSTSFROMACCPERCOSTCENTERBEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `TSFROMACCPERCOSTCENTERBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.FATHERID,
       ...
       t.IMPORTDATETIME,
       t.RETRYNR
FROM   DB2ADMIN.COSTSFROMACCPERCOSTCENTERBEAN t
