# DB2ADMIN.PRODUCTSPECIALIZEDGREIGEBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 86632
...
| 2 | `REEDHEIGHT` | DECIMAL(15,5) |  |  |  |  |
| 3 | `SELVAGEREEDHEIGHT` | DECIMAL(15,5) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `PRODUCTSPECIALIZEDGREIGEBEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `PRDSPECIALIZEDGREIGEBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.FATHERID,
       ...
       t.IMPLASTUPDATEUSER,
       t.IMPORTDATETIME
FROM   DB2ADMIN.PRODUCTSPECIALIZEDGREIGEBEAN t
