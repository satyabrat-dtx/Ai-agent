# DB2ADMIN.PRODUCTSPECIALIZEDYARNBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 86680
...
| 2 | `YARNCOUNTSYSTEMCODE` | CHAR(4) |  |  |  |  |
| 3 | `YARNCOUNT` | DECIMAL(7,2) |  |  |  |  |
| 4 | `NUMBEROFPLY` | DECIMAL(3,0) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `PRODUCTSPECIALIZEDYARNBEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `PRODUCTSPECIALIZEDYARNBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.FATHERID,
       ...
       t.IMPLASTUPDATEUSER,
       t.IMPORTDATETIME
FROM   DB2ADMIN.PRODUCTSPECIALIZEDYARNBEAN t
