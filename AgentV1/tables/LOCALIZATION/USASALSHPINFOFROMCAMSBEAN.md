# DB2ADMIN.USASALSHPINFOFROMCAMSBEAN

> **DO NOT QUERY FOR BUSINESS DATA.** Integration staging/import mirror. Holds in-flight inbound rows, not the authoritative record, and relaxes the NOT NULL constraints of its twin. Do NOT use for business reporting.

- **Module**: `LOCALIZATION` (low confidence — table name starts with 'USA')
- **Roles**: `staging_mirror`
- **Columns**: 25
- **Primary key**: `IMPORTAUTOCOUNTER`
- **FK degree**: referenced by 0 constraint(s), references 0 constraint(s)
- **Source**: `DB2ADMIN_DDL.sql` line 86129

## Columns

| # | Column | Type | Null | Key | Tags | Meaning |
|---|--------|------|------|-----|------|---------|
| 0 | `IMPORTAUTOCOUNTER` | BIGINT | NOT NULL | PK | primary_key staging | Staging-row sequence number. Its presence marks a *BEAN import/staging mirror table. |
| 1 | `COMPANYCODE` | CHAR(3) |  |  | tenant_key | Company/legal-entity discriminator -- this schema's tenant key. Appears on 1,934 tables and is the leading primary-key column on most of them. Nearly every query should constrain it, and every join between company-scoped tables should include it. |
| 2 | `DUMMYCODE` | CHAR(1) |  |  |  |  |
| 3 | `PROVISIONALCOUNTERCODE` | CHAR(8) |  |  |  |  |
| 4 | `PROVISIONALCODE` | CHAR(15) |  |  |  |  |
| 5 | `SETONLYNOTNULL` | SMALLINT | NOT NULL |  |  |  |
| 6 | `NUMBERPARCEL` | DECIMAL(5,0) |  |  |  |  |
| 7 | `WEIGHTUNITOFMEASURECODE` | CHAR(3) |  |  |  |  |
| 8 | `WEIGHTGROSS` | DECIMAL(15,5) |  |  |  |  |
| 9 | `WEIGHTNET` | DECIMAL(15,5) |  |  |  |  |
| 10 | `WEIGHTREALNET` | DECIMAL(15,5) |  |  |  |  |
| 11 | `VOLUMEUNITOFMEASURECODE` | CHAR(3) |  |  |  |  |
| 12 | `VOLUME` | DECIMAL(15,5) |  |  |  |  |
| 13 | `TRACKINGNUMBER` | CHAR(25) |  |  |  |  |
| 14 | `WSOPERATION` | INTEGER | NOT NULL |  | staging | Requested web-service operation for a staging row (integration inbox pattern). |
| 15 | `IMPORTSTATUS` | INTEGER | NOT NULL |  | staging | Staging-row processing status (integration inbox pattern). |
| 16 | `IMPCREATIONDATETIME` | TIMESTAMP |  |  |  |  |
| 17 | `IMPCREATIONUSER` | CHAR(25) |  |  |  |  |
| 18 | `IMPLASTUPDATEDATETIME` | TIMESTAMP |  |  |  |  |
| 19 | `IMPLASTUPDATEUSER` | CHAR(25) |  |  |  |  |
| 20 | `IMPORTDATETIME` | TIMESTAMP |  |  |  |  |
| 21 | `RETRYNR` | INTEGER | NOT NULL |  | staging | Retry attempt counter for a staging row. |
| 22 | `NEXTRETRY` | BIGINT | NOT NULL |  | staging | Next retry timestamp for a failed staging row. |
| 23 | `IMPORTID` | BIGINT | NOT NULL |  |  |  |
| 24 | `IMPOPERATIONUSER` | CHAR(25) |  |  |  |  |

## References (this table → parent) — 0

_None._

## Referenced by (child → this table) — 0

_None._

## Starter query

```sql
SELECT t.IMPORTAUTOCOUNTER,
       t.COMPANYCODE,
       t.DUMMYCODE,
       t.PROVISIONALCOUNTERCODE,
       t.PROVISIONALCODE,
       t.SETONLYNOTNULL,
       t.NUMBERPARCEL,
       t.WEIGHTUNITOFMEASURECODE,
       t.WEIGHTGROSS,
       t.WEIGHTNET,
       t.WEIGHTREALNET,
       t.VOLUMEUNITOFMEASURECODE
FROM   DB2ADMIN.USASALSHPINFOFROMCAMSBEAN t
WHERE  t.COMPANYCODE = ?   -- tenant key: always constrain
FETCH FIRST 100 ROWS ONLY;
```
