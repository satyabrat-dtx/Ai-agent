...
- **Source**: `DB2ADMIN_DDL.sql` line 222127
...
| 5 | `ITEMTYPECODE` | CHAR(3) | NOT NULL |  |  |  |
| 6 | `DIVISIONCODE` | CHAR(3) | NOT NULL |  |  | Division within a company; second-level organisational discriminator. |
| 7 | `ENDDATE` | DATE | NOT NULL |  |  |  |
| 8 | `BUSINESSAREACODE` | CHAR(50) | NOT NULL |  |  |  |
| 9 | `LOGICALWAREHOUSECOMPANYCODE` | CHAR(3) | NOT NULL |  |  |  |
| 10 | `LOGICALWAREHOUSECODE` | CHAR(8) | NOT NULL |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGCONSUMPTIONHEADER.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.FLAG,
       ...
       t.POSTINGDATE
FROM   DB2ADMIN.LOGCONSUMPTIONHEADER t
