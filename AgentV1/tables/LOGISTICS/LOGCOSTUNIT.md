...
- **Source**: `DB2ADMIN_DDL.sql` line 101101
...
| 18 | `LOGUSER` | CHAR(25) |  |  | audit | User responsible for the audited change (change-log table). |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGCOSTUNIT.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
