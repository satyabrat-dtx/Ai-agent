# DB2ADMIN.LOGCUSTOMERSUPPLIERDATA
...
- **Source**: `DB2ADMIN_DDL.sql` line 72793
...
| 2 | `CODE` | CHAR(8) | NOT NULL |  |  | Business (natural) key of a master-data table, typically the last primary-key column. |
| 3 | `BUSINESSPARTNERNUMBERID` | DECIMAL(8,0) |  |  |  |  |
| 4 | `WAREHOUSECODE` | CHAR(8) |  |  |  |  |
| 5 | `COMPANYBANKIDENTIFIER` | DECIMAL(3,0) |  |  |  |  |
| 6 | `CURRENCYCODE` | CHAR(4) |  |  |  |  |
...
| 16 | `WAREHOUSECOMPANYCODE` | CHAR(3) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGCUSTOMERSUPPLIERDATA.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.COMPANYCODE,
       ...
       t.LASTUPDATEUSER
FROM   DB2ADMIN.LOGCUSTOMERSUPPLIERDATA t
