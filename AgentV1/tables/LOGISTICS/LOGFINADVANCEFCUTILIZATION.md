# DB2ADMIN.LOGFINADVANCEFCUTILIZATION
...
- **Source**: `DB2ADMIN_DDL.sql` line 201955
...
| 13 | `FINADVFCUTILOGMGMNT` | SMALLINT | NOT NULL |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINADVANCEFCUTILIZATION.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.FINADVANCECOMPANYCODE,
       ...
       t.MARKETRATE,
       t.BANKCHARGESUSD
FROM   DB2ADMIN.LOGFINADVANCEFCUTILIZATION t
