# DB2ADMIN.LOGFINADVANCEPCUTILIZATION
...
- **Source**: `DB2ADMIN_DDL.sql` line 202003
...
|---|--------|------|------|-----|------|---------|
| 0 | `FINADVANCECOMPANYCODE` | CHAR(3) | NOT NULL |  |  |  |
| 1 | `FINADVANCEBUSINESSUNITCODE` | CHAR(10) | NOT NULL |  |  |  |
| 2 | `FINADVANCEADVANCENUMBER` | CHAR(10) | NOT NULL |  |  |  |
| 3 | `BANKREFERENCENO` | CHAR(20) |  |  |  |  |
| 4 | `REFERENCEDATE` | DATE |  |  |  |  |
| 5 | `PCBALANCE` | DECIMAL(18,5) |  |  |  |  |
| 6 | `ADJFORBILL` | DECIMAL(18,5) |  |  |  |  |
| 7 | `PCNOLETTERNO` | CHAR(5) |  |  |  |  |
| 8 | `PCGENERALLEDGERCOMPANYCODE` | CHAR(3) |  |  |  |  |
| 9 | `PCGENERALLEDGERCODE` | CHAR(20) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINADVANCEPCUTILIZATION.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.FINADVANCECOMPANYCODE,
       ...
       t.CREATIONDATETIME,
       t.CREATIONUSER
FROM   DB2ADMIN.LOGFINADVANCEPCUTILIZATION t
