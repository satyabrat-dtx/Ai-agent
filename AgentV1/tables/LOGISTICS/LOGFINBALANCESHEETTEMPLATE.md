# DB2ADMIN.LOGFINBALANCESHEETTEMPLATE
...
- **Source**: `DB2ADMIN_DDL.sql` line 230066
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINBALANCESHEETTEMPLATE.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
FROM   DB2ADMIN.LOGFINBALANCESHEETTEMPLATE t
