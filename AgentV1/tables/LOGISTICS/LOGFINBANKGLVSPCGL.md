...
- **Source**: `DB2ADMIN_DDL.sql` line 224334
...
| 1 | `BANKGLCOMPANYCODE` | CHAR(3) | NOT NULL |  |  |  |
| 2 | `BANKGLCODE` | CHAR(20) | NOT NULL |  |  |  |
| 3 | `PACKINGCREDITGLCOMPANYCODE` | CHAR(3) | NOT NULL |  |  |  |
| 4 | `PACKINGCREDITGLCODE` | CHAR(20) | NOT NULL |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINBANKGLVSPCGL.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.COMPANYCODE,
       ...
       t.ABSUNIQUEID
FROM   DB2ADMIN.LOGFINBANKGLVSPCGL t
