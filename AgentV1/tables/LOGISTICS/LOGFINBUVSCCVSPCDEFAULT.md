# DB2ADMIN.LOGFINBUVSCCVSPCDEFAULT
...
- **Source**: `DB2ADMIN_DDL.sql` line 222872
...
| 1 | `BUSSINESSUNITCODE` | CHAR(10) | NOT NULL |  |  |  |
| 2 | `PROFITCENTERCODE` | CHAR(10) |  |  |  |  |
| 3 | `COSTCENTERCOMPANYCODE` | CHAR(3) |  |  |  |  |
| 4 | `COSTCENTERCODE` | CHAR(20) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINBUVSCCVSPCDEFAULT.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.COMPANYCODE,
       ...
       t.ABSUNIQUEID
FROM   DB2ADMIN.LOGFINBUVSCCVSPCDEFAULT t
