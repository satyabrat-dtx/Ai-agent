# DB2ADMIN.LOGFINDEPRECIATIONRATEMASTER
...
- **Source**: `DB2ADMIN_DDL.sql` line 227630
...
| 1 | `COMPANYITACT` | CHAR(1) | NOT NULL |  |  |  |
| 2 | `DEPRRATECODE` | CHAR(5) | NOT NULL |  |  |  |
...
| 7 | `NOOFMONTH` | DECIMAL(20,0) | NOT NULL |  |  |  |
| 8 | `ADDITIONDEPRESIONJOB` | DECIMAL(5,2) | NOT NULL |  |  |  |
| 9 | `ADDDEPAPPDATE` | DATE |  |  |  |  |
| 10 | `DEPRMETHOD` | CHAR(1) | NOT NULL |  |  |  |
| 11 | `EFFECTIVEDATEFROM` | DATE | NOT NULL |  |  |  |
| 12 | `EFFECTIVEDATETO` | DATE |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINDEPRECIATIONRATEMASTER.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.COMPANYCODE,
       ...
       t.DEPRMETHOD,
       t.EFFECTIVEDATEFROM
FROM   DB2ADMIN.LOGFINDEPRECIATIONRATEMASTER t
