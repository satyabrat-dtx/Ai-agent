...
- **Source**: `DB2ADMIN_DDL.sql` line 202425
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINEXPEDCHARGES.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.COMPANYCODE,
       ...
       t.DUEDATEFROM,
       t.DUEDATETO
FROM   DB2ADMIN.LOGFINEXPEDCHARGES t
