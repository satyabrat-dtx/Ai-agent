# DB2ADMIN.LOGFINEXPPCUTILIZEDFC
...
- **Source**: `DB2ADMIN_DDL.sql` line 202978
...
| 2 | `FINFCNOFCNO` | CHAR(10) | NOT NULL |  |  |  |
| 3 | `LINENUMBER` | DECIMAL(7,0) | NOT NULL |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINEXPPCUTILIZEDFC.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.FINPACKINGCREDITCOMPANYCODE,
       ...
       t.ABSUNIQUEID
FROM   DB2ADMIN.LOGFINEXPPCUTILIZEDFC t
