# DB2ADMIN.LOGFINGLVSCOSTCENTERMAPPING
...
- **Source**: `DB2ADMIN_DDL.sql` line 223827
...
| 3 | `TEMPLATECODE` | CHAR(3) | NOT NULL |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINGLVSCOSTCENTERMAPPING.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
FROM   DB2ADMIN.LOGFINGLVSCOSTCENTERMAPPING t
