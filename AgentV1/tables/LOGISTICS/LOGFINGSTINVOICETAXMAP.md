# DB2ADMIN.LOGFINGSTINVOICETAXMAP
...
- **Source**: `DB2ADMIN_DDL.sql` line 230389
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINGSTINVOICETAXMAP.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
- child `LOGFINGSTINVOICETAXMAPDETAIL`.`FATHERID` → this table's `ABSUNIQUEID` (high confidence)
...
FROM   DB2ADMIN.LOGFINGSTINVOICETAXMAP t
