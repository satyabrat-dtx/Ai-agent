# DB2ADMIN.LOGFININTERBUBANKCHARGES
...
- **Source**: `DB2ADMIN_DDL.sql` line 225618
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFININTERBUBANKCHARGES.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.FININTERUNITTRNCOMPANYCODE,
       ...
       t.CREATIONDATETIME,
       t.CREATIONUSER
FROM   DB2ADMIN.LOGFININTERBUBANKCHARGES t
