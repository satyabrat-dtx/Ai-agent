...
- **Source**: `DB2ADMIN_DDL.sql` line 226402
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINLNSCHEDULE.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
- child `LOGFINLNSCHEDULEDETAIL`.`FATHERID` → this table's `ABSUNIQUEID` (high confidence)
...
SELECT t.FINLOANMASTERCOMPANYCODE,
       ...
       t.LASTUPDATEUSER
FROM   DB2ADMIN.LOGFINLNSCHEDULE t
