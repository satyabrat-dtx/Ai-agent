...
- **Source**: `DB2ADMIN_DDL.sql` line 230570
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINLOANINSURANCE.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.FINLOANMASTERCOMPANYCODE,
       ...
       t.CREATIONDATETIME,
       t.CREATIONUSER
FROM   DB2ADMIN.LOGFINLOANINSURANCE t
