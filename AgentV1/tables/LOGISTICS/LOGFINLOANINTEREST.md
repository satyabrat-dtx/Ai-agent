...
- **Source**: `DB2ADMIN_DDL.sql` line 224527
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINLOANINTEREST.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
- child `LOGFINLOANINTERESTCALC`.`FATHERID` → this table's `ABSUNIQUEID` (medium confidence)
- child `LOGFINLOANINTERESTSUBSIDY`.`FATHERID` → this table's `ABSUNIQUEID` (medium confidence)
...
SELECT t.FINLOANMASTERCOMPANYCODE,
       ...
       t.CREATIONDATETIME,
       t.CREATIONUSER
FROM   DB2ADMIN.LOGFINLOANINTEREST t
