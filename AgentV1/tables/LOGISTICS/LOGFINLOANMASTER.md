# DB2ADMIN.LOGFINLOANMASTER

- **Module**: `LOGISTICS` (high confidence — table name starts with 'LOG')
- **Roles**: `change_log`, `child_of_implicit_parent`
- **Columns**: 54
- **Primary key**: `ABSUNIQUEID`, `LOGTIMESTAMP`, `UUID`
- **FK degree**: referenced by 0 constraint(s), references 0 constraint(s)
- **Source**: `DB2ADMIN_DDL.sql` line 231102

> Row-level change-log/audit table.

## Columns

| # | Column | Type | Null | Key | Tags | Meaning |
|---|--------|------|------|-----|------|---------|
| 0 | `COMPANYCODE` | CHAR(3) | NOT NULL |  | tenant_key | Company/legal-entity discriminator -- this schema's tenant key. Appears on 1,934 tables and is the leading primary-key column on most of them. Nearly every query should constrain it, and every join between company-scoped tables should include it. |
| 1 | `LTEUGENGROUPTYPECOMPANYCODE` | CHAR(3) | NOT NULL |  |  |  |
| 2 | `LTYPEUSERGENERICGROUPTYPECODE` | CHAR(3) | NOT NULL |  |  |  |
| 3 | `LOANTYPECODE` | CHAR(10) | NOT NULL |  |  |  |
| 4 | `BUSINESSUNITCODE` | CHAR(10) |  |  |  |  |
| 5 | `LOANNO` | CHAR(10) | NOT NULL |  |  |  |
| 6 | `LOANDATE` | DATE |  |  |  |  |
| 7 | `LOANNUMBER` | CHAR(20) |  |  |  |  |
| 8 | `LENDERCOMPANYCODE` | CHAR(3) |  |  |  |  |
| 9 | `LENDERCODE` | CHAR(20) |  |  |  |  |
| 10 | `APPLICATIONDATE` | DATE |  |  |  |  |
| 11 | `TERMLOANGLCOMPANYCODE` | CHAR(3) |  |  |  |  |
| 12 | `TERMLOANGLCODE` | CHAR(20) |  |  |  |  |
| 13 | `LOANGLDESCRIPTION` | VARCHAR(200) |  |  |  |  |
| 14 | `CURRENCYCODE` | CHAR(4) |  |  |  |  |
| 15 | `PROJECTCOST` | DECIMAL(18,5) |  |  |  |  |
| 16 | `MARGIN` | DECIMAL(5,2) |  |  |  |  |
| 17 | `SANCTIONAMT` | DECIMAL(18,5) |  |  |  |  |
| 18 | `PAIDAMT` | DECIMAL(18,5) |  |  |  |  |
| 19 | `AVAILEDAMT` | DECIMAL(18,5) |  |  |  |  |
| 20 | `LCDISBURSEMENTAMT` | DECIMAL(18,5) |  |  |  |  |
| 21 | `DATEOFAGREEMENT` | DATE |  |  |  |  |
| 22 | `SANCTIONDATE` | DATE |  |  |  |  |
| 23 | `CHARGECREATED` | CHAR(1) |  |  |  |  |
| 24 | `SUBSIDY` | DECIMAL(5,2) |  |  |  |  |
| 25 | `NARRATION` | VARCHAR(500) |  |  |  |  |
| 26 | `SECURITYNATURE` | CHAR(1) |  |  |  |  |
| 27 | `LOANPERIODINMONTHS` | DECIMAL(3,0) |  |  |  |  |
| 28 | `REPAYMENT` | CHAR(1) |  |  |  |  |
| 29 | `MORATORIUMINMONTHS` | DECIMAL(3,0) |  |  |  |  |
| 30 | `SECURITYDATEOFCREATION` | DATE |  |  |  |  |
| 31 | `NOOFREPAYMENTS` | DECIMAL(3,0) |  |  |  |  |
| 32 | `FIRSTDISBURSEMENTDATE` | DATE |  |  |  |  |
| 33 | `REPAYMENTSTARTDATE` | DATE |  |  |  |  |
| 34 | `INTERESTSCHEDULE` | CHAR(1) |  |  |  |  |
| 35 | `STATUS` | CHAR(1) |  |  |  |  |
| 36 | `TUFSELIGIBLE` | DECIMAL(18,5) |  |  |  |  |
| 37 | `TUFSNOTELIGIBLE` | DECIMAL(18,5) |  |  |  |  |
| 38 | `SCHAMT` | CHAR(2) |  |  |  |  |
| 39 | `INTERESTGLCOMPANYCODE` | CHAR(3) |  |  |  |  |
| 40 | `INTERESTGLCODE` | CHAR(20) |  |  |  |  |
| 41 | `CREATIONDATETIME` | TIMESTAMP |  |  | audit | Local-time creation timestamp (audit). |
| 42 | `CREATIONUSER` | CHAR(50) |  |  | audit | User who created the row (audit). |
| 43 | `LASTUPDATEDATETIME` | TIMESTAMP |  |  | audit | Local-time last-modification timestamp (audit). |
| 44 | `LASTUPDATEUSER` | CHAR(50) |  |  | audit | User who last modified the row (audit). |
| 45 | `CREATIONDATETIMEUTC` | TIMESTAMP |  |  | audit | UTC creation timestamp (audit). Prefer this over the local-time twin for comparisons across companies. |
| 46 | `LASTUPDATEDATETIMEUTC` | TIMESTAMP |  |  | audit | UTC last-modification timestamp (audit). |
| 47 | `ABSUNIQUEID` | BIGINT | NOT NULL | PK | primary_key surrogate_id | Framework-assigned surrogate row id (BIGINT). Present on most tables. NO foreign key in this schema references it, but it is the target of the implicit FATHERID parent link. Not part of the primary key. |
| 48 | `LOGTIMESTAMP` | BIGINT | NOT NULL | PK | primary_key audit | When the audited change was recorded (change-log table). |
| 49 | `LOGOPERATION` | INTEGER | NOT NULL |  | audit | Kind of audited change -- insert/update/delete (change-log table). |
| 50 | `LOGUSER` | CHAR(50) |  |  | audit | User responsible for the audited change (change-log table). |
| 51 | `UUID` | VARCHAR(50) | NOT NULL | PK | primary_key | Externally-generated unique identifier, used for integration correlation. |
| 52 | `FATHERID` | BIGINT | NOT NULL |  | implicit_parent_ref | Implicit parent-row pointer holding the parent table's ABSUNIQUEID. NOT declared as a foreign key anywhere -- the parent table is resolved by the application, not the schema. Join as: parent.ABSUNIQUEID = child.FATHERID. |
| 53 | `LOGUPDATEDFIELDS` | CLOB(1000000) |  |  |  |  |

## References (this table → parent) — 0

_None._

## Referenced by (child → this table) — 0

_None._

## Implicit links (NOT declared in the DDL — inferred)

- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINLOANMASTER.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Starter query

```sql
SELECT t.COMPANYCODE,
       t.LTEUGENGROUPTYPECOMPANYCODE,
       t.LTYPEUSERGENERICGROUPTYPECODE,
       t.LOANTYPECODE,
       t.BUSINESSUNITCODE,
       t.LOANNO,
       t.LOANDATE,
       t.LOANNUMBER,
       t.LENDERCOMPANYCODE,
       t.LENDERCODE,
       t.APPLICATIONDATE,
       t.TERMLOANGLCOMPANYCODE
FROM   DB2ADMIN.LOGFINLOANMASTER t
WHERE  t.COMPANYCODE = ?   -- tenant key: always constrain
FETCH FIRST 100 ROWS ONLY;
```
