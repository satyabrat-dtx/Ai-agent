# DB2ADMIN.LOGFINLOANOUTSTANDING
...
- **Source**: `DB2ADMIN_DDL.sql` line 230722
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINLOANOUTSTANDING.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.COMPANYCODE,
       ...
       t.OUTSTANDINGAMOUNT,
       t.CREATIONDATETIME
FROM   DB2ADMIN.LOGFINLOANOUTSTANDING t
