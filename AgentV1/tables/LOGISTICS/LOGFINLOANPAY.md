...
- **Source**: `DB2ADMIN_DDL.sql` line 223172
...
| 4 | `LOANCODELOANNO` | CHAR(10) | NOT NULL |  |  |  |
| 5 | `SCHEDULENO` | INTEGER | NOT NULL |  |  |  |
| 6 | `SCHEDULESEQNO` | INTEGER | NOT NULL |  |  |  |
| 7 | `SCHEDULEDATE` | DATE |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINLOANPAY.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
