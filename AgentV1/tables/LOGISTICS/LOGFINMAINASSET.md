...
- **Source**: `DB2ADMIN_DDL.sql` line 229356
...
| 2 | `ASSETUGENGROUPTYPECOMPANYCODE` | CHAR(3) | NOT NULL |  |  |  |
| 3 | `ASSETUSERGENERICGROUPTYPECODE` | CHAR(3) | NOT NULL |  |  |  |
...
| 9 | `ASSETCOUNTERCOMPANYCODE` | CHAR(3) |  |  |  |  |
| 10 | `ASSETCOUNTERCODE` | CHAR(8) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINMAINASSET.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
- child `LOGFINMAINASSETGLMAPPING`.`FATHERID` → this table's `ABSUNIQUEID` (medium confidence)
...
SELECT t.COMPANYCODE,
       ...
       t.ASSETCOUNTERCODE,
       t.ACTIVATION
FROM   DB2ADMIN.LOGFINMAINASSET t
