# DB2ADMIN.LOGFINMOTORPREMIUMCHART
...
- **Source**: `DB2ADMIN_DDL.sql` line 227278
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINMOTORPREMIUMCHART.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
- child `LOGFINMOTORPREMIUMCHARTDETAIL`.`FATHERID` → this table's `ABSUNIQUEID` (high confidence)
...
SELECT t.COMPANYCODE,
       ...
       t.ABSUNIQUEID,
       t.LOGTIMESTAMP
FROM   DB2ADMIN.LOGFINMOTORPREMIUMCHART t
