# DB2ADMIN.LOGFINPOADVPAYMENTBANKCHARGES
...
- **Source**: `DB2ADMIN_DDL.sql` line 224373
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINPOADVPAYMENTBANKCHARGES.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.FINPOADVANCEPROPOSALCMYCODE,
       ...
       t.CREATIONDATETIME,
       t.CREATIONUSER
FROM   DB2ADMIN.LOGFINPOADVPAYMENTBANKCHARGES t
