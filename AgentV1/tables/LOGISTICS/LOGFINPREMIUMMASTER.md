...
- **Source**: `DB2ADMIN_DDL.sql` line 231050
...
| 16 | `VALIDFROMDATE` | DATE | NOT NULL |  |  |  |
| 17 | `VALIDTODATE` | DATE |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINPREMIUMMASTER.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.COMPANYCODE,
       ...
       t.OI,
       t.NETRATE
FROM   DB2ADMIN.LOGFINPREMIUMMASTER t
