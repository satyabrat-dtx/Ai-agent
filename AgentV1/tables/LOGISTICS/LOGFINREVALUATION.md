...
- **Source**: `DB2ADMIN_DDL.sql` line 226346
...
| 4 | `REVALUATIONDATE` | DATE | NOT NULL |  |  |  |
| 5 | `POSTINGDATE` | DATE |  |  |  |  |
| 6 | `REPOSTINGDATE` | DATE |  |  |  |  |
| 7 | `FXGNGLACCCOMPANYCODE` | CHAR(3) |  |  |  |  |
| 8 | `FXGNGLACCCODE` | CHAR(20) |  |  |  |  |
| 9 | `PROCESSTYPE` | CHAR(1) | NOT NULL |  |  |  |
| 10 | `FXLSGLACCCOMPANYCODE` | CHAR(3) |  |  |  |  |
| 11 | `FXLSGLACCCODE` | CHAR(20) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINREVALUATION.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
- child `LOGFINREVALUATIONCHILD`.`FATHERID` → this table's `ABSUNIQUEID` (medium confidence)
...
SELECT t.COMPANYCODE,
       ...
       t.FXLSGLACCCODE
FROM   DB2ADMIN.LOGFINREVALUATION t
