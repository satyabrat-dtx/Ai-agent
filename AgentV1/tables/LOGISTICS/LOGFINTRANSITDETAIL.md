...
- **Source**: `DB2ADMIN_DDL.sql` line 222989
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINTRANSITDETAIL.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.FINPOLICYMASTERCOMPANYCODE,
       ...
       t.TODATE,
       t.AMOUNT
FROM   DB2ADMIN.LOGFINTRANSITDETAIL t
