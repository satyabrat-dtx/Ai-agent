...
- **Source**: `DB2ADMIN_DDL.sql` line 229925
...
| 2 | `UGGUSERGENERICGROUPTYPECODE` | CHAR(3) | NOT NULL |  |  |  |
| 3 | `UGGCODE` | CHAR(10) | NOT NULL |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFINUGGVSGLMAPPING.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
FROM   DB2ADMIN.LOGFINUGGVSGLMAPPING t
