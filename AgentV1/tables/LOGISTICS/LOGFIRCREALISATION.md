...
- **Source**: `DB2ADMIN_DDL.sql` line 203712
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGFIRCREALISATION.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.COMPANYCODE,
       ...
       t.FIRCAMOUNT,
       t.CREATIONDATETIME
FROM   DB2ADMIN.LOGFIRCREALISATION t
