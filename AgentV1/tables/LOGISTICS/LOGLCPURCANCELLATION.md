...
- **Source**: `DB2ADMIN_DDL.sql` line 219433
...
| 4 | `LINENUMBER` | DECIMAL(18,5) | NOT NULL |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGLCPURCANCELLATION.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
FROM   DB2ADMIN.LOGLCPURCANCELLATION t
