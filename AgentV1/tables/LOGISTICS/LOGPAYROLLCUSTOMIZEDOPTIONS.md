# DB2ADMIN.LOGPAYROLLCUSTOMIZEDOPTIONS
...
- **Source**: `DB2ADMIN_DDL.sql` line 222710
...
| 3 | `SUPPLIERCOUNTERCOMPANYCODE` | CHAR(3) |  |  |  |  |
| 4 | `SUPPLIERCOUNTERCODE` | CHAR(8) |  |  |  |  |
| 5 | `LOGFORALLENTITIES` | SMALLINT | NOT NULL |  |  |  |
...
| 19 | `SAPINTEGRATION` | SMALLINT | NOT NULL |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGPAYROLLCUSTOMIZEDOPTIONS.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
FROM   DB2ADMIN.LOGPAYROLLCUSTOMIZEDOPTIONS t
