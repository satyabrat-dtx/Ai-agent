# DB2ADMIN.LOGPERIODCLOSURECOSTACCOUNT
...
- **Source**: `DB2ADMIN_DDL.sql` line 224621
...
| 1 | `PERPERIODIZEDCALENDARTYPECODE` | CHAR(10) | NOT NULL |  |  |  |
| 2 | `PERIODPERIODIZEDCALENDARYEAR` | DECIMAL(4,0) | NOT NULL |  |  |  |
| 3 | `PERIODCODE` | DECIMAL(3,0) | NOT NULL |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGPERIODCLOSURECOSTACCOUNT.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.COMPANYCODE,
       ...
       t.ABSUNIQUEID
FROM   DB2ADMIN.LOGPERIODCLOSURECOSTACCOUNT t
