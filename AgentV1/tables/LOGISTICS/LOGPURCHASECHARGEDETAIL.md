# DB2ADMIN.LOGPURCHASECHARGEDETAIL
...
- **Source**: `DB2ADMIN_DDL.sql` line 116624
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGPURCHASECHARGEDETAIL.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.PURPRCLINEPURPRCLISTCMYCODE,
       ...
       t.CHARGESSUBCODE01,
       t.CHARGETYPE
FROM   DB2ADMIN.LOGPURCHASECHARGEDETAIL t
