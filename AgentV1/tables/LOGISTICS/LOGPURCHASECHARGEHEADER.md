# DB2ADMIN.LOGPURCHASECHARGEHEADER
...
- **Source**: `DB2ADMIN_DDL.sql` line 116675
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGPURCHASECHARGEHEADER.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.PURCHASEPRICELISTCOMPANYCODE,
       ...
       t.CHARGETYPE,
       t.VALUE
FROM   DB2ADMIN.LOGPURCHASECHARGEHEADER t
