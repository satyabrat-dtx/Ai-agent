# DB2ADMIN.LOGPURCHASEORDERCHARGE
...
- **Source**: `DB2ADMIN_DDL.sql` line 53104
...
- `FATHERID` → **LOGPURCHASEORDER**.`ABSUNIQUEID` (high confidence — name = 'LOGPURCHASEORDER' + known child suffix 'CHARGE')
  - JOIN predicate: `LOGPURCHASEORDERCHARGE.FATHERID = LOGPURCHASEORDER.ABSUNIQUEID`
...
SELECT t.PURCHASEORDERCOMPANYCODE,
       ...
       t.SIGN,
       t.CALCULATIONTYPE
FROM   DB2ADMIN.LOGPURCHASEORDERCHARGE t
