# DB2ADMIN.LOGPURMANACCTMPDEFINITION
...
- **Source**: `DB2ADMIN_DDL.sql` line 113493
...
| 18 | `ITEMTYPEAFICOMPANYCODE` | CHAR(3) |  |  |  |  |
...
| 31 | `ORDITEMGRPSTDORDGROUPTYPECODE` | CHAR(3) |  |  |  |  |
| 32 | `ORDERITEMGROUPCODE` | CHAR(3) |  |  |  |  |
...
| 49 | `RFPARTNERCUSTOMERSUPPLIERCODE` | CHAR(8) |  |  |  |  |
| 50 | `RFPRNGRPSTDORDERGROUPTYPECODE` | CHAR(3) |  |  |  |  |
| 51 | `REFERENCEPARTNERGROUPCODE` | CHAR(3) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGPURMANACCTMPDEFINITION.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.COMPANYCODE,
       ...
       t.DIVISIONCODE,
       t.ORDERTEMPLATECODE
FROM   DB2ADMIN.LOGPURMANACCTMPDEFINITION t
