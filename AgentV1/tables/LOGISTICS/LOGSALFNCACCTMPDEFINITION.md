# DB2ADMIN.LOGSALFNCACCTMPDEFINITION
...
- **Source**: `DB2ADMIN_DDL.sql` line 113720
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGSALFNCACCTMPDEFINITION.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
FROM   DB2ADMIN.LOGSALFNCACCTMPDEFINITION t
