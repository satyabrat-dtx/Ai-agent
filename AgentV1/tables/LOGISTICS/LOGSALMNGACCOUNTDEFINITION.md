# DB2ADMIN.LOGSALMNGACCOUNTDEFINITION
...
- **Source**: `DB2ADMIN_DDL.sql` line 113795
...
| 18 | `TAXCODE` | CHAR(3) |  |  |  |  |
| 19 | `ORDERLINETEMPLATECODE` | CHAR(3) |  |  |  |  |
| 20 | `ITEMTYPEAFICOMPANYCODE` | CHAR(3) |  |  |  |  |
...
| 33 | `ORDITEMGRPSTDORDGROUPTYPECODE` | CHAR(3) |  |  |  |  |
| 34 | `ORDERITEMGROUPCODE` | CHAR(3) |  |  |  |  |
...
| 51 | `RFPARTNERCUSTOMERSUPPLIERCODE` | CHAR(8) |  |  |  |  |
| 52 | `RFPRNGRPSTDORDERGROUPTYPECODE` | CHAR(3) |  |  |  |  |
| 53 | `REFERENCEPARTNERGROUPCODE` | CHAR(3) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGSALMNGACCOUNTDEFINITION.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.COMPANYCODE,
       ...
       t.DIVISIONCODE
FROM   DB2ADMIN.LOGSALMNGACCOUNTDEFINITION t
