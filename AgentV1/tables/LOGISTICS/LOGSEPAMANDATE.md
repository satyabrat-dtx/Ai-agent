...
- **Source**: `DB2ADMIN_DDL.sql` line 104583
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGSEPAMANDATE.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.COMPANYCODE,
       ...
       t.CREATIONDATETIME,
       t.CREATIONUSER
FROM   DB2ADMIN.LOGSEPAMANDATE t
