# DB2ADMIN.LOGSERVICETRANSACTIONTEMPLATE
...
- **Source**: `DB2ADMIN_DDL.sql` line 205960
...
| 18 | `INTRASTATTRANSACTIONNATURECODE` | CHAR(2) |  |  |  |  |
| 19 | `CHECKSERVICETRANSACTIONCODE` | CHAR(20) |  |  |  |  |
| 20 | `DOSERVICETRNSTOCKTRNDIST` | SMALLINT | NOT NULL |  |  |  |
| 21 | `ISTRANSFERLINKTOISSUE` | SMALLINT | NOT NULL |  |  |  |
| 22 | `SERVICETRNSTOCKTRNDISTPLYCODE` | CHAR(20) |  |  |  |  |
| 23 | `DOSERVICETRNSRVTRNDIST` | SMALLINT | NOT NULL |  |  |  |
| 24 | `SERVICETRNSRVTRNDISTPLYCODE` | CHAR(20) |  |  |  |  |
| 25 | `BUDGETCONTROL` | SMALLINT | NOT NULL |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGSERVICETRANSACTIONTEMPLATE.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
FROM   DB2ADMIN.LOGSERVICETRANSACTIONTEMPLATE t
