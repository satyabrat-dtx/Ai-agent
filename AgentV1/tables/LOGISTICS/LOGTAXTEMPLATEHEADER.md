...
- **Source**: `DB2ADMIN_DDL.sql` line 219348
...
| 5 | `EFFECTIVEFROMDATE` | DATE | NOT NULL |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LOGTAXTEMPLATEHEADER.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.COMPANYCODE,
       ...
       t.ROUNDOFFAMOUNT,
       t.CREATIONDATETIME
FROM   DB2ADMIN.LOGTAXTEMPLATEHEADER t
