...
- **Source**: `DB2ADMIN_DDL.sql` line 90769
...
| 2 | `HOLIDAYMONTH` | INTEGER | NOT NULL |  |  |  |
| 3 | `HOLIDAY` | INTEGER | NOT NULL |  |  |  |
| 4 | `HOLIDAYDESCRIPTION` | VARCHAR(200) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `CALENDARHOLIDAYBEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `CALENDARHOLIDAYBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.FATHERID,
       ...
       t.IMPLASTUPDATEUSER,
       t.IMPORTDATETIME
FROM   DB2ADMIN.CALENDARHOLIDAYBEAN t
