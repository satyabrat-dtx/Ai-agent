...
- **Source**: `DB2ADMIN_DDL.sql` line 90818
...
| 2 | `NUMBERWORKINGDAY` | CHAR(2) |  |  |  |  |
| 3 | `NUMBERGROUPSHIFT` | INTEGER | NOT NULL |  |  |  |
| 4 | `NUMBERSHIFT` | INTEGER | NOT NULL |  |  |  |
| 5 | `STARTSHIFT` | TIME |  |  |  |  |
| 6 | `ENDSHIFT` | TIME |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `CALENDARSHIFTBEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `CALENDARSHIFTBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
FROM   DB2ADMIN.CALENDARSHIFTBEAN t
