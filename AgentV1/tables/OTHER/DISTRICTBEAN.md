...
- **Source**: `DB2ADMIN_DDL.sql` line 115896
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `DISTRICTBEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `DISTRICTBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
