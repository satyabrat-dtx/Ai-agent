# DB2ADMIN.EXTERNALPRCLISTDEFCMTBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 98484
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `EXTERNALPRCLISTDEFCMTBEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `EXTERNALPRCLISTDEFCMTBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.REPORTTYPE,
       ...
       t.IMPCREATIONUSER
FROM   DB2ADMIN.EXTERNALPRCLISTDEFCMTBEAN t
