# DB2ADMIN.INTDOCUMENTGROUPCOMMENTBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 66984
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `INTDOCUMENTGROUPCOMMENTBEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `INTDOCGROUPCOMMENTBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.REPORTTYPE,
       ...
       t.CANCELED,
       t.WSOPERATION
FROM   DB2ADMIN.INTDOCUMENTGROUPCOMMENTBEAN t
