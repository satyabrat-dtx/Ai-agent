# DB2ADMIN.INTDOCUMENTLINEBLOCKSBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 67285
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `INTDOCUMENTLINEBLOCKSBEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `INTDOCUMENTLINEBLOCKSBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.FATHERID,
       ...
       t.UNBLOCKINGTYPE
FROM   DB2ADMIN.INTDOCUMENTLINEBLOCKSBEAN t
