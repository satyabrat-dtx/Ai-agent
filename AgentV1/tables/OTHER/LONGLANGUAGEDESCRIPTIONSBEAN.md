# DB2ADMIN.LONGLANGUAGEDESCRIPTIONSBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 215376
...
| 3 | `SUMMARIZEDDESCRIPTION` | CHAR(200) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LONGLANGUAGEDESCRIPTIONSBEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `GLANGUAGEDESCRIPTIONSBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.FATHERID,
       ...
       t.IMPLASTUPDATEUSER
FROM   DB2ADMIN.LONGLANGUAGEDESCRIPTIONSBEAN t
