# DB2ADMIN.PRDPRIMARYUOMCONVERSIONBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 44876
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `PRDPRIMARYUOMCONVERSIONBEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `PRDPRMUOMCONVERSIONBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
FROM   DB2ADMIN.PRDPRIMARYUOMCONVERSIONBEAN t
