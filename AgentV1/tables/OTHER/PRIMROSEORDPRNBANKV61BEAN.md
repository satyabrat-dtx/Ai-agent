# DB2ADMIN.PRIMROSEORDPRNBANKV61BEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 43567
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `PRIMROSEORDPRNBANKV61BEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `PRIMROSEORDPRNBANKV61BEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.FATHERID,
       ...
       t.BIC,
       t.IBAN
FROM   DB2ADMIN.PRIMROSEORDPRNBANKV61BEAN t
