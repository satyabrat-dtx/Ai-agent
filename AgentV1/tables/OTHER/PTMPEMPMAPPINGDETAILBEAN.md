# DB2ADMIN.PTMPEMPMAPPINGDETAILBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 172947
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `PTMPEMPMAPPINGDETAILBEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `PTMPEMPMAPPINGDETAILBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.FATHERID,
       ...
       t.IMPLASTUPDATEUSER
FROM   DB2ADMIN.PTMPEMPMAPPINGDETAILBEAN t
