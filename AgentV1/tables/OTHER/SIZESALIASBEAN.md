...
- **Source**: `DB2ADMIN_DDL.sql` line 83179
...
| 2 | `CODE` | CHAR(40) |  |  |  | Business (natural) key of a master-data table, typically the last primary-key column. |
| 3 | `MARKETCODE` | CHAR(10) |  |  |  |  |
| 4 | `TOBEPRINTED` | SMALLINT | NOT NULL |  |  |  |
...
- `FATHERID` → **SIZES**.`ABSUNIQUEID` (high confidence — name = 'SIZES' + known child suffix 'ALIAS')
  - JOIN predicate: `SIZESALIASBEAN.FATHERID = SIZES.ABSUNIQUEID`

## Indexes

- `SIZESALIASBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.FATHERID,
       ...
       t.IMPLASTUPDATEUSER,
       t.IMPORTDATETIME
FROM   DB2ADMIN.SIZESALIASBEAN t
