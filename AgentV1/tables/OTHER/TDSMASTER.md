...
- **Roles**: `business_data`
- **Columns**: 16
- **Primary key**: `COMPANYCODE`, `TYPEUSERGENERICGROUPTYPECODE`, `TYPECODE`, `CODE`, `ITAXCODE`
- **FK degree**: referenced by 0 constraint(s), references 2 constraint(s)
- **Source**: `DB2ADMIN_DDL.sql` line 124952
...
| 1 | `TEUSGENGROUPTYPECOMPANYCODE` | CHAR(3) | NOT NULL | FK | foreign_key |  |
| 2 | `TYPEUSERGENERICGROUPTYPECODE` | CHAR(3) | NOT NULL | PK FK | primary_key foreign_key |  |
| 3 | `TYPECODE` | CHAR(10) | NOT NULL | PK FK | primary_key foreign_key |  |
| 4 | `CODE` | CHAR(6) | NOT NULL | PK | primary_key | Business (natural) key of a master-data table, typically the last primary-key column. |
| 5 | `ITAXCODE` | CHAR(3) | NOT NULL | PK | primary_key |  |
...
|---|---|---|---|---|---|
| `COMPANY_COMPANY` | `COMPANYCODE` | [`COMPANY`](../CORE_MASTER/COMPANY.md) | `CODE` | RESTRICT | `TDSMASTER.COMPANYCODE = COMPANY.CODE` |
| `USERGENERICGROUP_TYPE` | `TEUSGENGROUPTYPECOMPANYCODE`, `TYPEUSERGENERICGROUPTYPECODE`, `TYPECODE` | [`USERGENERICGROUP`](../CORE_MASTER/USERGENERICGROUP.md) | `USERGENGROUPTYPECOMPANYCODE`, `USERGENERICGROUPTYPECODE`, `CODE` | RESTRICT | `TDSMASTER.TEUSGENGROUPTYPECOMPANYCODE = USERGENERICGROUP.USERGENGROUPTYPECOMPANYCODE AND TDSMASTER.TYPEUSERGENERICGROUPTYPECODE = USERGENERICGROUP.USERGENERICGROUPTYPECODE AND TDSMASTER.TYPECODE = USERGENERICGROUP.CODE` |
...
- `TDSMASTERUID` (ABSUNIQUEID)

## Starter query

```sql
SELECT t.COMPANYCODE,
       t.TEUSGENGROUPTYPECOMPANYCODE,
       t.TYPEUSERGENERICGROUPTYPECODE,
       t.TYPECODE,
       t.CODE,
       t.ITAXCODE,
       t.LONGDESCRIPTION,
