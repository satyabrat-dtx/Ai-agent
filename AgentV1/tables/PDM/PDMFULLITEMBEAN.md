...
- **Source**: `DB2ADMIN_DDL.sql` line 75225
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `PDMFULLITEMBEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `PDMFULLITEMBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.FATHERID,
       ...
       t.SUBCODE07,
       t.SUBCODE08
FROM   DB2ADMIN.PDMFULLITEMBEAN t
