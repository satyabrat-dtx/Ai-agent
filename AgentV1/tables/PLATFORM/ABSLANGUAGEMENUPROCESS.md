# DB2ADMIN.ABSLANGUAGEMENUPROCESS
...
- **Source**: `DB2ADMIN_DDL.sql` line 14712
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `ABSLANGUAGEMENUPROCESS.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.FATHERID,
       ...
       t.NAME,
       t.DESCRIPTION
FROM   DB2ADMIN.ABSLANGUAGEMENUPROCESS t
