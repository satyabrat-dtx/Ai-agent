...
- **Source**: `DB2ADMIN_DDL.sql` line 26668
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `ADTRANSLATEDOPTIONS.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.FATHERID,
       t.LANGUAGECODE,
       t.OPTIONS
FROM   DB2ADMIN.ADTRANSLATEDOPTIONS t
