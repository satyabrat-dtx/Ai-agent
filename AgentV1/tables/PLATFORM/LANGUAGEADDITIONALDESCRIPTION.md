# DB2ADMIN.LANGUAGEADDITIONALDESCRIPTION
...
- **Source**: `DB2ADMIN_DDL.sql` line 214535
...
| 2 | `ADDITIONALDESCRIPTION` | VARCHAR(300) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LANGUAGEADDITIONALDESCRIPTION.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
FROM   DB2ADMIN.LANGUAGEADDITIONALDESCRIPTION t
