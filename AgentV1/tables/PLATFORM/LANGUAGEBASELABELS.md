...
- **Source**: `DB2ADMIN_DDL.sql` line 3270
...
| 2 | `STATISTICALGROUP` | VARCHAR(80) |  |  |  |  |
| 3 | `CONTAINER` | VARCHAR(80) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LANGUAGEBASELABELS.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.FATHERID,
       ...
       t.STATISTICALGROUP,
       t.CONTAINER
FROM   DB2ADMIN.LANGUAGEBASELABELS t
