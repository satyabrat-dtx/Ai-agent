# DB2ADMIN.LANGUAGEPROTOTYPELABELS
...
- **Source**: `DB2ADMIN_DDL.sql` line 211516
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LANGUAGEPROTOTYPELABELS.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.FATHERID,
       t.LANGUAGECODE,
       t.PROJECT
FROM   DB2ADMIN.LANGUAGEPROTOTYPELABELS t
