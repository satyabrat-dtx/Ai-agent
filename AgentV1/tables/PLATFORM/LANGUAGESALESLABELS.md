...
- **Source**: `DB2ADMIN_DDL.sql` line 670
...
| 2 | `PALLETLABEL` | VARCHAR(80) |  |  |  |  |
| 3 | `PARCELGROUPLABEL` | VARCHAR(80) |  |  |  |  |
| 4 | `PARCELLABEL` | VARCHAR(80) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `LANGUAGESALESLABELS.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.
...
SELECT t.FATHERID,
       ...
       t.PARCELGROUPLABEL,
       t.PARCELLABEL
FROM   DB2ADMIN.LANGUAGESALESLABELS t
