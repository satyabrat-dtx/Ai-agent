# DB2ADMIN.PATTERNREPETITIONBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 91411
...
| 2 | `REPETITIONLEVEL` | CHAR(2) |  |  |  |  |
| 3 | `FROMCOL` | CHAR(5) |  |  |  |  |
| 4 | `TOCOL` | CHAR(5) |  |  |  |  |
| 5 | `FIELDVALUE` | CHAR(5) |  |  |  |  |
| 6 | `NOOFREPS` | DECIMAL(5,0) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `PATTERNREPETITIONBEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `PATTERNREPETITIONBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
FROM   DB2ADMIN.PATTERNREPETITIONBEAN t
