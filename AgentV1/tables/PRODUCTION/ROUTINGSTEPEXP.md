# DB2ADMIN.ROUTINGSTEPEXP

- **Module**: `PRODUCTION` (high confidence — table name starts with 'ROUTING')
- **Roles**: `business_data`
- **Columns**: 11
- **Primary key**: `ENVIRONMENTCODE`, `EXPROUTINGCOMPANYCODE`, `EXPROUTINGNUMBERID`, `EXPSEQUENCE`, `EXPSUBSEQUENCE`, `UNIQUEIDPK`
- **FK degree**: referenced by 0 constraint(s), references 0 constraint(s)
- **Source**: `DB2ADMIN_DDL.sql` line 89858
...
| 1 | `EXPROUTINGCOMPANYCODE` | CHAR(3) | NOT NULL | PK | primary_key |  |
| 2 | `EXPROUTINGNUMBERID` | DECIMAL(11,0) | NOT NULL | PK | primary_key |  |
...
- `ROUTINGSTEPEXPUID` (ABSUNIQUEID)

## Starter query

```sql
SELECT t.ENVIRONMENTCODE,
       t.EXPROUTINGCOMPANYCODE,
       t.EXPROUTINGNUMBERID,
       t.EXPSEQUENCE,
       t.EXPSUBSEQUENCE,
       t.OPERATIONTYPE,
       t.STATUS,
       ...
       t.FATHERABSUNIQUEID,
       t.EVENTEXPDATETIME
FROM   DB2ADMIN.ROUTINGSTEPEXP t
