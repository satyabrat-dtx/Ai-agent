# DB2ADMIN.PURCHASEDISCOUNTDETAILBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 93282
...
| 6 | `CALCULATEDVALUE` | DECIMAL(18,5) |  |  |  |  |
| 7 | `INITIALDATE` | DATE |  |  |  |  |
| 8 | `FINALDATE` | DATE |  |  |  |  |
| 9 | `PAYMENTMETHODCODE` | CHAR(3) |  |  |  |  |
| 10 | `DISCOUNTTYPE` | CHAR(2) |  |  |  |  |
| 11 | `VALUE` | DECIMAL(18,5) |  |  |  |  |
| 12 | `DISCOUNTCURRENCYCODE` | CHAR(4) |  |  |  |  |
| 13 | `SIGN` | CHAR(2) |  |  |  |  |
| 14 | `TAXAPPLICATIONTYPE` | CHAR(2) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `PURCHASEDISCOUNTDETAILBEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `PURCHASEDISCOUNTDETAILBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.FATHERID,
       ...
       t.DISCOUNTTYPE,
       t.VALUE
FROM   DB2ADMIN.PURCHASEDISCOUNTDETAILBEAN t
