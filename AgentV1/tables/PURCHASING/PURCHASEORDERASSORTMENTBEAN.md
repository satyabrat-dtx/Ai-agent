# DB2ADMIN.PURCHASEORDERASSORTMENTBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 59036
...
- `FATHERID` → **PURCHASEORDER**.`ABSUNIQUEID` (high confidence — name = 'PURCHASEORDER' + known child suffix 'ASSORTMENT')
  - JOIN predicate: `PURCHASEORDERASSORTMENTBEAN.FATHERID = PURCHASEORDER.ABSUNIQUEID`

## Indexes

- `PURORDERASSORTMENTBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.FATHERID,
       ...
       t.SUBCODE04,
       t.SUBCODE05
FROM   DB2ADMIN.PURCHASEORDERASSORTMENTBEAN t
