# DB2ADMIN.PURCHASEORDERBLOCKSBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 59116
...
- `FATHERID` → **PURCHASEORDER**.`ABSUNIQUEID` (high confidence — name = 'PURCHASEORDER' + known child suffix 'BLOCKS')
  - JOIN predicate: `PURCHASEORDERBLOCKSBEAN.FATHERID = PURCHASEORDER.ABSUNIQUEID`

## Indexes

- `PURCHASEORDERBLOCKSBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.FATHERID,
       ...
       t.UNBLOCKINGTYPE,
       t.UNBLOCKINGUSER
FROM   DB2ADMIN.PURCHASEORDERBLOCKSBEAN t
