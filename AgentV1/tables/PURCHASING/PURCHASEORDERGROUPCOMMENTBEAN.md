# DB2ADMIN.PURCHASEORDERGROUPCOMMENTBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 59638
...
| 4 | `ORIGIN` | INTEGER | NOT NULL |  |  |  |
| 5 | `CODE` | CHAR(12) |  |  |  | Business (natural) key of a master-data table, typically the last primary-key column. |
| 6 | `COMMENTTEXT` | LONG VARCHAR |  |  |  |  |
| 7 | `COMMENTTYPE` | CHAR(2) |  |  |  |  |
...
- `FATHERID` → **PURCHASEORDER**.`ABSUNIQUEID` (medium confidence — name = 'PURCHASEORDER' + recurring fragment 'GROUPCOMMENT' (seen in 6 tables))
  - JOIN predicate: `PURCHASEORDERGROUPCOMMENTBEAN.FATHERID = PURCHASEORDER.ABSUNIQUEID`

## Indexes

- `PURORDERGROUPCOMMENTBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
FROM   DB2ADMIN.PURCHASEORDERGROUPCOMMENTBEAN t
