# DB2ADMIN.PURCHASEORDERLINEBLOCKSBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 59701
...
- `FATHERID` → **PURCHASEORDERLINE**.`ABSUNIQUEID` (high confidence — name = 'PURCHASEORDERLINE' + known child suffix 'BLOCKS')
  - JOIN predicate: `PURCHASEORDERLINEBLOCKSBEAN.FATHERID = PURCHASEORDERLINE.ABSUNIQUEID`

## Indexes

- `PURORDERLINEBLOCKSBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.FATHERID,
       ...
       t.UNBLOCKINGTYPE,
       t.UNBLOCKINGUSER
FROM   DB2ADMIN.PURCHASEORDERLINEBLOCKSBEAN t
