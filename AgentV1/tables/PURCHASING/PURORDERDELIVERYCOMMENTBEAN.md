# DB2ADMIN.PURORDERDELIVERYCOMMENTBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 59516
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `PURORDERDELIVERYCOMMENTBEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `PURORDDELIVERYCOMMENTBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.REPORTTYPE,
       ...
       t.IMPCREATIONUSER
FROM   DB2ADMIN.PURORDERDELIVERYCOMMENTBEAN t
