# DB2ADMIN.SALESORDERDELIVERYCOMMENTBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 60329
...
- `FATHERID` → **SALESORDERDELIVERY**.`ABSUNIQUEID` (high confidence — name = 'SALESORDERDELIVERY' + known child suffix 'COMMENT')
  - JOIN predicate: `SALESORDERDELIVERYCOMMENTBEAN.FATHERID = SALESORDERDELIVERY.ABSUNIQUEID`

## Indexes

- `SALORDDELIVERYCOMMENTBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
SELECT t.REPORTTYPE,
       ...
       t.IMPCREATIONUSER
FROM   DB2ADMIN.SALESORDERDELIVERYCOMMENTBEAN t
