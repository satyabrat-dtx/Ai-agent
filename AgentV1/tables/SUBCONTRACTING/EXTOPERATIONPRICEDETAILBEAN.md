# DB2ADMIN.EXTOPERATIONPRICEDETAILBEAN
...
- **Source**: `DB2ADMIN_DDL.sql` line 98667
...
| 4 | `BREAKDOWNLIMIT` | DECIMAL(18,5) |  |  |  |  |
| 5 | `PRICEFROMHEADER` | SMALLINT | NOT NULL |  |  |  |
| 6 | `PRICETYPE` | CHAR(2) |  |  |  |  |
...
- `FATHERID` → **parent not derivable**. FATHERID present but no parent table is derivable from the name. Join as `EXTOPERATIONPRICEDETAILBEAN.FATHERID = <parent>.ABSUNIQUEID`, where `<parent>` must come from the query context.

## Indexes

- `EXTOPERATIONPRCDETAILBEANXMT` (IMPORTSTATUS, RETRYNR, IMPORTAUTOCOUNTER, NEXTRETRY)
...
FROM   DB2ADMIN.EXTOPERATIONPRICEDETAILBEAN t
