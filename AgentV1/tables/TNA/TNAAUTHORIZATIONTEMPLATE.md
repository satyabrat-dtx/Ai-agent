# DB2ADMIN.TNAAUTHORIZATIONTEMPLATE

- **Module**: `TNA` (low confidence — table name starts with 'TNA')
- **Roles**: `business_data`
- **Columns**: 16
- **Primary key**: `COMPANYCODE`, `CODE`, `UNIQUEID`
- **FK degree**: referenced by 3 constraint(s), references 2 constraint(s)
- **Source**: `DB2ADMIN_DDL.sql` line 195538

## Columns

| # | Column | Type | Null | Key | Tags | Meaning |
|---|--------|------|------|-----|------|---------|
| 0 | `UIXMLPATH` | VARCHAR(50) | NOT NULL |  |  |  |
| 1 | `UIXML` | VARCHAR(54) | NOT NULL |  |  |  |
| 2 | `COMPANYCODE` | CHAR(3) | NOT NULL | PK FK | primary_key foreign_key tenant_key | Company/legal-entity discriminator -- this schema's tenant key. Appears on 1,934 tables and is the leading primary-key column on most of them. Nearly every query should constrain it, and every join between company-scoped tables should include it. |
| 3 | `CODE` | CHAR(3) | NOT NULL | PK | primary_key | Business (natural) key of a master-data table, typically the last primary-key column. |
| 4 | `UNIQUEID` | BIGINT | NOT NULL | PK | primary_key |  |
| 5 | `LONGDESCRIPTION` | VARCHAR(200) | NOT NULL |  | description | Long human-readable label. |
| 6 | `SHORTDESCRIPTION` | VARCHAR(80) |  |  | description | Short human-readable label. |
| 7 | `SEARCHDESCRIPTION` | VARCHAR(120) |  |  | description | Normalised/uppercased label used for lookup and search screens. |
| 8 | `CREATIONDATETIME` | TIMESTAMP |  |  | audit | Local-time creation timestamp (audit). |
| 9 | `CREATIONUSER` | CHAR(50) |  |  | audit | User who created the row (audit). |
| 10 | `LASTUPDATEDATETIME` | TIMESTAMP |  |  | audit | Local-time last-modification timestamp (audit). |
| 11 | `LASTUPDATEUSER` | CHAR(50) |  |  | audit | User who last modified the row (audit). |
| 12 | `CREATIONDATETIMEUTC` | TIMESTAMP |  |  | audit | UTC creation timestamp (audit). Prefer this over the local-time twin for comparisons across companies. |
| 13 | `LASTUPDATEDATETIMEUTC` | TIMESTAMP |  |  | audit | UTC last-modification timestamp (audit). |
| 14 | `ABSUNIQUEID` | BIGINT | NOT NULL |  | surrogate_id | Framework-assigned surrogate row id (BIGINT). Present on most tables. NO foreign key in this schema references it, but it is the target of the implicit FATHERID parent link. Not part of the primary key. |
| 15 | `ENTITYJNDINAME` | VARCHAR(100) |  | FK | foreign_key |  |

## References (this table → parent) — 2

| Constraint | Local columns | → Table | → Columns | ON DELETE | JOIN predicate |
|---|---|---|---|---|---|
| `COMPANY_COMPANY` | `COMPANYCODE` | [`COMPANY`](../CORE_MASTER/COMPANY.md) | `CODE` | RESTRICT | `TNAAUTHORIZATIONTEMPLATE.COMPANYCODE = COMPANY.CODE` |
| `TNAPARENTENTITY_ENTITY` | `ENTITYJNDINAME` | [`TNAPARENTENTITY`](../TNA/TNAPARENTENTITY.md) | `JNDINAME` | RESTRICT | `TNAAUTHORIZATIONTEMPLATE.ENTITYJNDINAME = TNAPARENTENTITY.JNDINAME` |

## Referenced by (child → this table) — 3

| Constraint | Child table | Child columns | JOIN predicate |
|---|---|---|---|
| `TNAAUTHORIZATIONTEMPLATE_AUTHORIZATIONTEMPLATE` | [`TNAACTIVITYLINKEDENTITY`](../TNA/TNAACTIVITYLINKEDENTITY.md) | `COMPANY`, `AUTHORIZATIONTEMPLATECODE`, `UNIQUEID` | `TNAACTIVITYLINKEDENTITY.COMPANY = TNAAUTHORIZATIONTEMPLATE.COMPANYCODE AND TNAACTIVITYLINKEDENTITY.AUTHORIZATIONTEMPLATECODE = TNAAUTHORIZATIONTEMPLATE.CODE AND TNAACTIVITYLINKEDENTITY.UNIQUEID = TNAAUTHORIZATIONTEMPLATE.UNIQUEID` |
| `TNAAUTHORIZATIONTEMPLATE_AUTHORIZATIONFIELDS` | [`TNAAUTHORIZATIONFIELDS`](../TNA/TNAAUTHORIZATIONFIELDS.md) | `TNAAUTHORIZATIONTMPCMYCODE`, `TNAAUTHORIZATIONTEMPLATECODE`, `TNAAUTHORIZATIONTMPUNIQUEID` | `TNAAUTHORIZATIONFIELDS.TNAAUTHORIZATIONTMPCMYCODE = TNAAUTHORIZATIONTEMPLATE.COMPANYCODE AND TNAAUTHORIZATIONFIELDS.TNAAUTHORIZATIONTEMPLATECODE = TNAAUTHORIZATIONTEMPLATE.CODE AND TNAAUTHORIZATIONFIELDS.TNAAUTHORIZATIONTMPUNIQUEID = TNAAUTHORIZATIONTEMPLATE.UNIQUEID` |
| `TNAAUTHORIZATIONTEMPLATE_AUTHORIZATIONRULES` | [`TNAAUTHORIZATIONRULE`](../TNA/TNAAUTHORIZATIONRULE.md) | `TNAAUTHORIZATIONTMPCMYCODE`, `TNAAUTHORIZATIONTEMPLATECODE`, `TNAAUTHORIZATIONTMPUNIQUEID` | `TNAAUTHORIZATIONRULE.TNAAUTHORIZATIONTMPCMYCODE = TNAAUTHORIZATIONTEMPLATE.COMPANYCODE AND TNAAUTHORIZATIONRULE.TNAAUTHORIZATIONTEMPLATECODE = TNAAUTHORIZATIONTEMPLATE.CODE AND TNAAUTHORIZATIONRULE.TNAAUTHORIZATIONTMPUNIQUEID = TNAAUTHORIZATIONTEMPLATE.UNIQUEID` |

## Indexes

- `TNAAUTHORIZATIONTEMPLATEUID` (ABSUNIQUEID)

## Starter query

```sql
SELECT t.UIXMLPATH,
       t.UIXML,
       t.COMPANYCODE,
       t.CODE,
       t.UNIQUEID,
       t.LONGDESCRIPTION,
       t.SHORTDESCRIPTION,
       t.SEARCHDESCRIPTION,
       t.CREATIONDATETIME,
       t.CREATIONUSER,
       t.LASTUPDATEDATETIME,
       t.LASTUPDATEUSER
FROM   DB2ADMIN.TNAAUTHORIZATIONTEMPLATE t
WHERE  t.COMPANYCODE = ?   -- tenant key: always constrain
FETCH FIRST 100 ROWS ONLY;
```
